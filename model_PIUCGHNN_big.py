...
from keras.layers import Input, MaxPooling2D, UpSampling2D, Dropout, Conv2D, Concatenate, Activation,Lambda
...
def conv_factory(x, concat_axis, nb_filter,
    ...
    x = Activation('relu')(x)
    x = Conv2D(nb_filter, (3, 3), dilation_rate=(2, 2),
...
def holoToRgbTF(holo_pred):
    ...
    F.add_aperture_from_arrayTF(holo, image_size=(X * mm, Y * mm))
    recon = F.compute_colors_atTF(-50 * mm)
    recon = tf.cast(recon, tf.float32)
    rgbGrayNor = noramlizationTF(recon)
    rgbGrayNor4D = tf.reshape(rgbGrayNor,[1, YN, XM, 1])
    return rgbGrayNor4D

# define model U-net modified with dense block
def PIUCGHModel():
    inputs = Input((64*1, 64*1, 1))
    print("inputs shape:", inputs.shape)

    conv1 = Conv2D(16, 3, activation='relu', padding='same', kernel_initializer='he_normal')(inputs)
    print("conv1 shape:", conv1.shape)
    db1 = denseblock(x=conv1, concat_axis=3, nb_layers=4, growth_rate=4, dropout_rate=0.5)
    ...
    db2 = denseblock(x=conv2, concat_axis=3, nb_layers=4, growth_rate=4, dropout_rate=0.5)
    ...
    db3 = denseblock(x=conv3, concat_axis=3, nb_layers=4, growth_rate=4, dropout_rate=0.5)
    ...
    db4 = denseblock(x=conv4, concat_axis=3, nb_layers=4, growth_rate=4, dropout_rate=0.5)
    ...
    db5 = denseblock(x=conv5, concat_axis=3, nb_layers=4, growth_rate=4, dropout_rate=0.5)
    ...
    db6 = denseblock(x=conv6, concat_axis=3, nb_layers=3, growth_rate=4, dropout_rate=0.5)
    ...
    db7 = denseblock(x=conv7, concat_axis=3, nb_layers=3, growth_rate=4, dropout_rate=0.5)
    ...
    db8 = denseblock(x=conv8, concat_axis=3, nb_layers=3, growth_rate=4, dropout_rate=0.5)
    ...
    db9 = denseblock(x=conv9, concat_axis=3, nb_layers=3, growth_rate=4, dropout_rate=0.5)
    ...
    rgbGrayNor4D=Lambda(holoToRgbTF)(conv11)

    model = Model(inputs=inputs, outputs=rgbGrayNor4D)
